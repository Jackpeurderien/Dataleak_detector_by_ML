...
def call_prediction(title, log_file, model, to_matrix_model, pipeline):
    ...
    prediction = pipeline(log_file, model, to_matrix_model)
    prediction = list(prediction)
    col1, col2, col3 = st.columns(3)
    col1.metric("Nombre de requetes Analysees", len(prediction))
    col2.metric("Nombre de requetes Suspectes", prediction.count(1))
    col3.metric("Nombre de requetes Saines", prediction.count(0))
